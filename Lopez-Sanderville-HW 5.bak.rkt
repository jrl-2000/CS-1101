...
#reader(lib "htdp-intermediate-reader.ss" "lang")((modname |Lopez-Sanderville-HW 5.bak|) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #t)))
...
;;count-alkaline-tributaries: String river-system -> Natural
...
#;(define (count-alkaline-tributaries ariv name)
    (... (river-name ariv)
         (river-ph ariv)
         (river-do ariv)
         (river-flow ariv)
...
#;(define (lor-fcn alor)
    (cond [(empty? alor) (...)]
          [(cons? alor) (...     (count-alkaline-tributaries (first alor) name)
                                 (lor-fcn (rest alor)))]))
...
(define (count-alkaline-tributaries-of ariv name)
  (- (count-alk (find-subsystem ariv name))
     (if (< 8.5 (river-ph ariv))
         1
         0)))


  
(define (count-alk ariv)
  (cond [(empty? (river-tributaries ariv))
         (if (< 8.5 (river-ph ariv))
             1
             0)]
        [else (+ (if (< 8.5 (river-ph ariv))
                     1
                     0)
                 (foldr + 0 (map count-alk (river-tributaries ariv))))])) 
...
(check-expect (count-alkaline-tributaries-of MISSOURI-SYSTEM "Gardner") 1)
